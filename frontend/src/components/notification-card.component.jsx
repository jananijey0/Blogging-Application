...
const NotificationCard =({data, index, notificationState}) =>{
    ...
    let {seen, type, reply, createdAt, comment, replied_on_comment, user,user: {personal_info: { profile_img, username, fullname}}, blog: {_id, blog_id,title}, _id : notification_id} = data;   
        ...
        <div className={`p-6 border-b border-grey ${!seen ? 'border-l-black border-l-2' : ''}`}>
            <div className="flex gap-5 mb-3">
                ...
                <div className="w-full">
                    <h1 className="font-medium text-xl text-dark-grey"> <span className="lg:inline-block hidden capitalize">{fullname}</span>
                    ...
                    {
                        type == "reply" ? <div className="p-4 mt-4 rounded-md"> <p>{replied_on_comment.comment}</p></div>:<Link to = {`/blog/${blog_id}`} className="font-medium text-dark-grey hover:underline line-clamp-1">{`"${title}"`}</Link>
                    }
                </div>
            </div>
            {
            ...
            {
                isReplying ? <div className="mt-8"> <NotificationCommentField _id = {_id} blog_author = {user} index = {index} replyingTo = {comment._id} setReplying = {setReplying} notification_id = {notification_id} notificationData ={notificationState} setNotifications ={setNotifications}/></div>:""
            }
            {
                reply ?
                <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                    <div>
                        ...
                        <div>
                            <h1 className="font-medium text-xl text-dark-grey"><Link to = {`/user/${author_username}`} className="mx-1 text-black underline">@{author_username}</Link>
                    ...
                    <button className="underline ml-14 mt-2 hover:text-black" onClick={(e) => handleDelete(comment._id,"reply",e.target)}>Delete</button>
                </div>:""
            }
        </div>
    )
